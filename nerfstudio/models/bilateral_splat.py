...
from bilags.lib_bilagrid import slice, BilateralGrid, total_variation_loss
...
@dataclass
class BilagsModelConfig(ModelConfig):
    """Splatfacto Model Config, nerfstudio's implementation of Gaussian Splatting"""

    _target: Type = field(default_factory=lambda: BilagsModel)
...
class BilagsModel(Model):
    ...
    def get_param_groups(self) -> Dict[str, List[Parameter]]:
        ...
        gps["bil_grids"] = list(self.bil_grids.parameters())
        ...
        if self.config.use_bilateral_grid and self.training:
            if camera.metadata is not None and "cam_idx" in camera.metadata:
                ...
                if cam_idx != 0:
                    ...
                    # prepare grid idx
                    # grid_idx = (
                    #     torch.ones((H, W), dtype=torch.long, device=self.device) * cam_idx
                    # )
                    # grid_idx = grid_idx.unsqueeze(-1)  # [H, W, 1]
                    # process rgb
                    out = slice(
                        ...
                        grid_idx=torch.tensor(
                            cam_idx, device=self.device, dtype=torch.long
                        ),
                    )
                    rgb = out["rgb"]
