    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            adam(
                ...
                gauss_scaling=self.gauss_scaling
            )
...
def _multi_tensor_adam(params: List[Tensor],
    ...
    for (device_params, device_grads, device_exp_avgs, device_exp_avg_sqs,
        ...
        if capturable:
            ...
            exp_abs = device_exp_avgs[0].abs()
            scaling = torch.ones_like(exp_abs)
            biggers = (exp_abs>torch.tensor(gauss_scaling)).squeeze()
            scaling[biggers] = (torch.tensor(gauss_scaling)/exp_abs)[biggers]
            # scaling = torch.tensor(gauss_scaling)
            device_exp_avgs = [x*scaling for i,x in enumerate(device_exp_avgs)]
