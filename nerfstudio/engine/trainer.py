        ...
        with TimeWriter(writer, EventName.TOTAL_TRAIN_TIME):
            ...
            for step in range(self._start_step, self._start_step + num_iterations):
                ...
                if step_check(step, self.config.logging.steps_per_log, run_at_zero=True):
                    ...
                    writer.put_scalar(name="Train PSNR", scalar=metrics_dict['psnr'], step=step)
                    ...
                    writer.put_scalar(
                        name="GPU Memory Allocated (MB)", scalar=torch.cuda.max_memory_allocated() / (1024**2), step=step
                    )
                    writer.put_scalar(
                        name="GPU Memory Reserved (MB)", scalar=torch.cuda.max_memory_reserved() / (1024**2), step=step
                    )
