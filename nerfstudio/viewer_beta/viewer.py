...
class Viewer:
    ...
    def update_camera_poses(self):
        ...
        for i, j in list(zip(idxs, list(range(len(idxs))))):
            # i is the key with which camera_handles is indexed into
            # j is the iterator number
            # both are numpy arrays
            c2w_orig = self.original_c2w[i]
            c2w_delta = c2ws_delta[j, ...]
            c2w = c2w_orig @ np.concatenate((c2w_delta, np.array([[0, 0, 0, 1]])), axis=0)
            R = vtf.SO3.from_matrix(c2w[:3, :3])  # type: ignore
            R = R @ vtf.SO3.from_x_radians(np.pi)
            self.camera_handles[i].position = c2w[:3, 3] * VISER_NERFSTUDIO_SCALE_RATIO
            self.camera_handles[i].wxyz = R.wxyz
