...
def get_ordered_poses_and_k_and_time(
    ...
    times: Optional[Float[Tensor, "num_poses 1"]],
    ...
    for _ in range(poses_num - 1):
        ...
        ordered_times = torch.cat((ordered_times, torch.unsqueeze(times[idx], 0)), dim=0) if times is not None else None
...
def get_interpolated_poses_many(
    ...
    return torch.tensor(traj, dtype=torch.float32), torch.tensor(k_interp, dtype=torch.float32), time_interp
