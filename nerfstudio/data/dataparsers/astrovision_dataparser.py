...
"""Data parser for AstroVision datasets."""
...
@dataclass
class AstroVisionDataParserConfig(DataParserConfig):
    """Nerfstudio dataset config"""

    _target: Type = field(default_factory=lambda: AstroVision)
    """target class to instantiate"""
    data: Path = Path()
    """Directory or explicit json file path specifying location of data."""
    #scale_factor: float = 0.01 / 2
    ...
    center_method: Literal["poses", "points", "focus", "none"] = "points"
...
@dataclass
class AstroVision(DataParser):
    """Nerfstudio DatasetParser"""

    config: AstroVisionDataParserConfig
    downscale_factor: Optional[int] = None
    ...
    def _generate_dataparser_outputs(self, split="train"):
        ...
        if self.config.center_method == "points":
            poses, transform_matrix = camera_utils.auto_orient_and_center_poses(
                poses,
                method=orientation_method,
                center_method="none",
            )
            pcd = np.asarray(o3d.io.read_point_cloud(os.path.join(self.config.data, "sparse_pc.ply")).points)
            vertices = torch.from_numpy(pcd).type(torch.float32)  # (N, 3)
            vertices = (transform_matrix[:, :3] @ vertices.T + transform_matrix[:, 3].reshape((3, 1))).T
            translation = torch.mean(vertices, axis=0)
        ...
        scale_factor = 1.0
        if self.config.center_method == "points":
            min_vertices, _ = vertices.min(axis=0)
            max_vertices, _ = vertices.max(axis=0)
            print(min_vertices)
            print(max_vertices)
            scale_factor *= 2.0 / (torch.max(max_vertices - min_vertices))
            aabb_scale_x, aabb_scale_y, aabb_scale_z = (max_vertices - min_vertices) * scale_factor / 2
            print(aabb_scale_x, aabb_scale_y, aabb_scale_z)
        else:
        ...
        print("dataparser_transform_matrix =")
        print(dataparser_transform_matrix)
