...
class PiganDataParser(DataParser):
    ...
    def _generate_dataparser_outputs(self, split="train"): 
        ...
        for num in meta:
            filepath = PurePath(num[self.config.data_type]['file_path'])
    ...
    def sample_camera_transform_matrix(self, n):
        ...
        """
        camera_origin, _, _ = _sample_camera_positions(n=n, r=self.config.radius, mode=self.config.pose_sample_mode)
        forward_vector = _normalize_vecs(-camera_origin)
        camera_transform_matrix = _create_cam2world_matrix(forward_vector, camera_origin)
