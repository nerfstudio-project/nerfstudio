...
ns-download nerfstudio --save-dir . --capture-name aspen

# 1 GPU (8192 rays per GPU per batch)
export CUDA_VISIBLE_DEVICES=0
ns-train nerfacto --machine.num-gpus 1 --pipeline.datamanager.train-num-rays-per-batch 8192 --data nerfstudio/aspen/
...
By have more GPUs in the training, you can allocate batch size to multiple GPUs and average their gradients.

```python
# 2 GPUs (4096 rays per GPU per batch, effectively 9182 rays per batch)
export CUDA_VISIBLE_DEVICES=0,1
ns-train nerfacto-big --machine.num-gpus 2 --pipeline.datamanager.train-num-rays-per-batch 4096 --data nerfstudio/aspen/
```
