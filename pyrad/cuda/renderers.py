...
from pyrad.cuda.rays import RaySamplesPacked
from pyrad.utils.math import components_from_spherical_harmonics


class RGBRendererPacked(nn.Module):
    ...
    def combine_rgb(
        cls,
        packed_indices: TensorType["num_rays", 3],
        rgb: TensorType[..., 3],
        weights: TensorType[..., 1],
        background_color: Optional[TensorType[3]] = None,
    ) -> TensorType["num_rays", 3]:
        """Composite samples along ray and render color image

        Args:
            packed_indices (TensorType["num_rays", 3]): Indices for recovering rays from packed samples.
            rgb (TensorType[..., 3]): RGB for each sample
            weights (TensorType[..., 1]): Weights for each sample
            background_color (TensorType[3], optional): Background color as RGB. Defaults to random.

        Returns:
            TensorType["num_rays", 3]: Outputs rgb values.
        """
    ...
    def forward(
        ...
        Returns:
            TensorType["num_rays", 3]: Outputs of rgb values.
        """
        raise NotImplementedError
...
class SHRendererPacked(nn.Module):
    ...
    def forward(
        ...
        Args:
            packed_indices (TensorType["num_rays", 3]): Indices for recovering rays from packed samples.
            sh (TensorType[..., "coeffs"]): Spherical hamonics coefficients for each sample
            directions: (TensorType[..., 3]): Sample direction
            weights (TensorType[..., 1]): Weights for each sample
...
class AccumulationRendererPacked(nn.Module):
...
class DepthRenderer(nn.Module):
    ...
    def forward(self, weights: TensorType[..., "num_samples", 1], ray_samples: RaySamplesPacked) -> TensorType[..., 1]:
        """Composite samples along ray and calculate disparities.

        Args:
            packed_indices (TensorType["num_rays", 3]): Indices for recovering rays from packed samples.
            weights (TensorType[..., "num_samples", 1]): Weights for each sample.
            ray_samples (RaySamplesPacked): Set of ray samples.
