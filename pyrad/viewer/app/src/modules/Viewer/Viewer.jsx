...
import { WebRtcContext } from '../WebRtc/WebRtc';
...
export default function ViewerState(props) {
  const state = {
    scene: null,
    controls: null,
    renderer: null,
  ...
  const handle_command = (cmd) => {
    ...
    // web rtc commands
      ...
      console.log('setting the answer');
  ...
  useEffect(() => {
    // componenet did mount things

    console.log('websocket in viewer');
    ...
    state.websocket.onmessage = (cmd) => handle_command(cmd.data);
    ...
    // GUI
    state.gui = new GUI();
    const scene_folder = state.gui.addFolder('Scene');
    scene_folder.open();
    state.scene_tree = new SceneNode(state.scene, scene_folder);
  ...
  //     send_training_state_over_websocket(props.paused);
  //   }

  //   // Choose render type
  //   if (prevProps.output_options !== props.output_options) {
  //     console.log(props.output_options);
  //     send_output_type_over_websocket(props.output_options);
  //   }

  //   // Set minimum render resolution
  //   if (prevProps.min_resolution !== props.min_resolution) {
  //     send_min_resolution_over_websocket(props.min_resolution.slice(0, -2));
  //   }

  //   // Set maximum render resolution
  //   if (prevProps.max_resolution !== props.max_resolution) {
  //     send_max_resolution_over_websocket(props.max_resolution.slice(0, -2));
  //   }
  // };
