...
def _generate_completion(script_path: pathlib.Path, shell: ShellType, completions_dir: pathlib.Path) -> bool:
    ...
    target_path = (
        completions_dir / shell / (script_path.name if shell == "bash" else "_" + script_path.name.replace(".", "_"))
    )
...
def _update_rc(
    ...
    if mode == "install":
        ...
        CONSOLE.log(f":person_gesturing_ok: Completions installed to {rc_path}. {_exclamation()} Open a new shell to try them out.")
