...
class CreateJSONCameraPath(bpy.types.Operator):
    ...
    def constructJsonObj(self):
        ...
        render_height = int(bpy.context.scene.render.resolution_y * (bpy.context.scene.render.resolution_percentage * 0.01))
        render_width = int(bpy.context.scene.render.resolution_x * (bpy.context.scene.render.resolution_percentage *0.01))
        render_fps = 1
        ...
        for i in range(len(self.transformed_cameraPath_mat)):
            camera_path_elem = {"camera_to_world": self.getListFromMatrixPath(self.transformed_cameraPath_mat[i]),"fov":self.fov_list[i], "aspect": 1}
            final_camera_path += [camera_path_elem]

        # construct keyframes
        keyframe_list = []
        for i in range(len(self.transformed_cameraPath_mat)):
            curr_properties = "[[\"FOV\"," + str(self.fov_list[i]) + "],[\"NAME\",\"Camera " + str(i) + "\"],[\"TIME\"," + str(i) + "]]"
            
            keyframe_elem = {"matrix": str(self.getListFromMatrixKeyframe(self.transformed_cameraPath_mat[i])),"fov":self.fov_list[i], "aspect": 1, "properties":curr_properties}
            keyframe_list += [keyframe_elem]

        overallJSON = {
         "keyframes": keyframe_list,
         "camera_type": cam_type, "render_height": render_height, "render_width": render_width,
         "camera_path": final_camera_path, "fps": render_fps, "seconds": render_seconds,
         "smoothness_value":smoothness_value, "is_cycle": is_cycle
         }
    ...
    def execute(self,context):
        ...
        params = (
        context.scene.NeRF,
        context.scene.JSONInputFilePath
        )
...
class ReadJSONinputCameraPath(bpy.types.Operator):
    ...
    def execute(self,context):
        ...
        params = (
        context.scene.NeRF,
        context.scene.NS_InputJSONFilePath
        )
...
INPUT_PROPERTIES = [
    ("JSONInputFilePath", bpy.props.StringProperty \
      (
      name = "JSON File Path",
      default = "//",
...
INPUT_PROPERTIES_NS_CAMERA = [
    ("NS_InputJSONFilePath", bpy.props.StringProperty \
      (
